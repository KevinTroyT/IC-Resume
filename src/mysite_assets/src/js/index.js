...
$(function() {
	...
	c.load("<div class='bef_comment writecode'>&nbsp;*hi,我的名字叫KK，是上海一家互联网公司的软件工程师</div>");
	c.load("<div class='bef_comment writecode'>&nbsp;*在ic上尝试发布一个简历</div>");
	...
	c.load("<div class='code writecode'>&nbsp;<span class='bef_key'>-webkit-transition</span>: <span class='bef_value'> all .3s</span>;</div>");
	...
	w.load("<p><div class='writecode'>唐弋洲，KK，Kevin Troy Thang</div>");
	w.load("<div class='writecode'>软件工程师(湖南宏洲网络科技有限公司，蜻蜓云（上海）)</div>");
	w.load("<div class='writecode'>区块链开发工程师，前端，全栈</div></p>");
	w.load("<h1 class='writecode'>联系我</h1>");
	w.load("<ul>");
	w.load("<li class='writecode'>mail:kkevin@hashonline.io</li>");
	w.load("<li class='writecode'>github:<a href='https://github.com/KevinTroyT' target='_blank'>https://github.com/KevinTroyT</a></li>");
	w.load("</ul>");
	w.load("<h1 class='writecode'>技能</h1>");
	w.load("<ul>");
	w.load("<li class='writecode'>熟练各种Web前端开发技术H5,CSS3,jQuery,React</li>");
	w.load("<li class='writecode'>精通围绕React的大多数框架 如Mobx,Redux。</li>");
	w.load("<li class='writecode'>熟练与各平台和硬件的技术对接（如: 微信接口，三方支付，系统整合）</li>");
	w.load("<li class='writecode'>掌握vue，小程序开发。</li>");
	w.load("<li class='writecode'>掌握Webpack等包管理工具。</li>");
	w.load("<li class='writecode'>UI框架精通 Antd Bootstarp Layui。</li>");
	w.load("<li class='writecode'>后端: 掌握Java,PHP,Python,Golang。</li>");
	w.load("<li class='writecode'>长期使用Git,JIRA等项目管理工具,Mac OS/Linux环境</li>");
	w.load("<li class='writecode'>关注最新技术栈，关注区块链技术栈，信息栈。</li>");
	w.load("<li class='writecode'>有管理开发团队经验。</li>");
	w.load("</ul>");
	c.load("<p><div class='comment writecode'>/**</div>");
	...
	c.load("<div class='comment writecode'>&nbsp;*开放点赞吧～</div>");
	c.load("<script>document.getElementById('thumbsUp').removeAttribute('disabled');</script>")
	c.load("<div class='comment writecode'>&nbsp;*/</div></p>");
...
var getNum = async () => {
	...
	document.getElementById("thumbsUp").innerText = "点赞 " + nums;
	return nums
}
document.getElementById("thumbsUp").onclick = async () => {
	document.getElementById("thumbsUp").setAttribute("disabled",true)
	await mysite.thumbsUp()
